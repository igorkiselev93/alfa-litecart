 ...
 * Throws if the element returns null (element not attached to DOM).
 * Allows empty string — use requireNonEmptyText when blank content is also invalid.
 */
export async function requireText(locator: Locator): Promise<string> {
  const text = await locator.textContent();
  if (text === null) {
    throw new Error(`Element has no text content: ${locator}`);
  }
  return text;
}
 ...
 * Throws if the attribute is missing (null).
 * Allows empty string — use requireNonEmptyAttribute when empty value is also invalid.
 */
